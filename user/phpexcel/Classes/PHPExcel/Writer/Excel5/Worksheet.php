...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
    ...
    /**
     * Write the GRadminET BIFF record. Must be used in conjunction with the
     * PRINTGRIDLINES record.
     */
    private function writeGradminet()
    {
        $record      = 0x0082;                        // Record identifier
        $length      = 0x0002;                        // Bytes to follow

        $fGradminet    = !$this->phpSheet->getPrintGridlines();     // Boolean flag

        $header      = pack("vv", $record, $length);
        $data        = pack("v", $fGradminet);
        $this->append($header . $data);
    }
